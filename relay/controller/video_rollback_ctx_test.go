...
// TestGoVideoRollbackPreConsumed_UsesLiveContext reproduces the silent refund-loss
// bug at video.go: the pre-consume rollback refund goroutine was spawned with the
...
func TestGoVideoRollbackPreConsumed_UsesLiveContext(t *testing.T) {
	gin.SetMode(gin.TestMode)

	gate := make(chan struct{})
	observed := make(chan error, 1)
	videoRollbackGateForTest = gate
	videoRollbackObservedCtxErrForTest = func(err error) {
		observed <- err
	}
	t.Cleanup(func() {
		videoRollbackGateForTest = nil
		videoRollbackObservedCtxErrForTest = nil
	})
	...
	goVideoRollbackPreConsumed(c, 0, 1207)
	...
	case err := <-observed:
		require.NoError(t, err,
			"video rollback refund must run on a live (non-cancelled) context; "+
